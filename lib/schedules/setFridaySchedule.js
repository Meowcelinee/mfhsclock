...
export default function setWeekdaySchedule() {
    ...
    for (let block in blocks.fridayBlocks) {
        ...
        const time = {
            start: tzHandler(
                startHour,
                blocks.fridayBlocks[block].start.split(' ')[1]
            ),
            end: tzHandler(
                endHour,
                blocks.fridayBlocks[block].end.split(' ')[1]
            ),
        };
        ...
        // prettier-ignore
        blockTimes.textContent = `${time.start.hour}:${blocks.fridayBlocks[block].start.split(' ')[0].split(':')[1]} ${time.start.time} -> ${time.end.hour}:${blocks.fridayBlocks[block].end.split(' ')[0].split(':')[1]} ${time.end.time}`;
